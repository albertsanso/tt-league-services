...
import org.cttelsamicsterrassa.data.core.domain.load.ResourceUploadService;
...
public class ImportResourceController {
    ...
    @Autowired
    public ImportResourceController(ResourceUploadService resourceUploadService) {
    ...
    public ResponseEntity<?> uploadZipFile(@RequestParam("file") MultipartFile file) {
        ...
        try {
            resourceUploadService.processZip(file.getOriginalFilename(), file.getBytes());
