...
/**
 * Canonical contract for the persisted "default import folder" administrator setting.
...
public final class ImportFolderSetting {
    ...
    public static final String NAME = "import-folder";
    ...
    /**
     * Validates a candidate value for the {@code IMPORT/import-folder} setting. The value must be
    ...
    public static void validate(String value) {
        if (value == null || value.isBlank()) {
            throw new IllegalArgumentException("Setting IMPORT/import-folder must not be blank");
        }
        try {
            Path.of(value.trim());
        } catch (InvalidPathException exception) {
            throw new IllegalArgumentException(
                    "Setting IMPORT/import-folder must be a valid filesystem path: " + value, exception);
        }
    }
}
